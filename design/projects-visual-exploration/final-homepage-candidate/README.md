...
**Signal Archive:** one row is a stage, and each project decides what happens on that stage. The page uses a cold editorial system of graphite type, narrow evidence labels, fine rules, and real output imagery. It deliberately avoids statistics strips, dashboards, a uniform card wall, and invented technology visuals.
...
Gemini 3.1 Pro Extended Thinking + Canvas was used as the primary visual synthesis partner with the locked Direction B reference and selected real assets. The first Canvas result was rejected because it invented runtime details for Codex Fix and changed the verified ESP32 API. `gemini-canvas-revised-source.html` records the corrected Canvas pass; `gemini-canvas-source.html` is the rejected first pass for audit only and is not a candidate. `qa/gemini-canvas-source.png` records the first visual pass before local asset paths were resolved.

The final local candidate preserves the revised Canvas thesis—editorial archive, project-specific stages, one flip, quiet CLI ending—while tightening real-asset handling, mobile anatomy, interaction semantics, and factual labels.
...
Accepted concept reference: `../qa/direction-b-1440.png`.
...
Browser-plugin capability was not available, so the documented Playwright fallback was used against `http://127.0.0.1:8792/`.
